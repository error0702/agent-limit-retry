...
async function confirm(q, yes) {
  if (yes) return true;
  if (!process.stdin.isTTY) { console.log(`${q}\n没有终端可以确认；要直接执行请加 --yes`); return false; }
...
export async function init({ yes = false } = {}) {
  ...
  if (!v && !hasZCode && !hasCodex) { console.log("没找到 Claude Code、Codex 或 ZCode。先装好其中一个。"); return 1; }
  if (!v) { const a = hasZCode ? await initZCode({ yes }) : 0; const b = hasCodex ? await initCodex({ yes }) : 0; return a || b; }
  const settings = readJSON(settingsPath(), {});
  const current = settings.statusLine;
  console.log(`agent-limit-retry 会做三件事（Claude Code ${v.text}）：
  1. 安装一个 Claude Code 插件：撞额度后，等重置了再按原 ID 叫醒被打断的子 Agent；额度快用完时先写交接文件
  2. 接管状态栏来读取额度百分比${current && !isOurs(current.command) ? "（你现有的状态栏会原样保留）" : "（会显示 ⏳ 5h xx% · 7d xx%）"}
  3. 修改前备份 ~/.claude/settings.json，随时可以 alr uninstall 还原`);
  if (!v.ok) console.log("  注意：你的 Claude Code 版本低于 2.1.234，建议先升级。");
  if (!(await confirm("开始安装？", yes))) return 1;

  copyApp();
  let r = claudeCmd(["plugin", "marketplace", "add", appDir()]);
  if (!r.ok && /already/i.test(r.out)) r = claudeCmd(["plugin", "marketplace", "update", NAME]);
  if (!r.ok) { console.log(`插件市场添加失败：${r.out}`); return 1; }
  r = claudeCmd(["plugin", "install", `${NAME}@${NAME}`]);
  if (!r.ok && !/already/i.test(r.out)) { console.log(`插件安装失败：${r.out}`); return 1; }
...
export async function initZCode({ yes = false, quiet = false } = {}) {
  ...
  if (!quiet) console.log(`检测到 ZCode。agent-limit-retry 会把插件目录登记到 ${zcode.configPath()}（改前备份），
  撞额度后在你下一句话时，把被打断的子 Agent 告诉主 Agent。`);
  if (zcode.installed(pluginDir)) { console.log("  ✓ ZCode 插件已登记"); return 0; }
  if (!(await confirm("登记到 ZCode？", yes))) return 1;
  copyApp();
  const r = zcode.register(pluginDir);
  console.log(`  ✓ ZCode 插件已登记${r.backup ? `（备份：${r.backup}）` : ""}`);
...
export async function initCodex({ yes = false, quiet = false } = {}) {
  if (!quiet) console.log(`检测到 Codex。agent-limit-retry 会用 codex plugin 命令安装插件（Codex 会把它复制到 ~/.codex/plugins/cache）。
  撞额度后，在你下一句话时把被打断的子 Agent 告诉主 Agent；alr run --agent codex 可以无人值守续跑。`);
  if (!(await confirm("安装到 Codex？", yes))) return 1;
  copyApp();
  let r = codexCmd(["plugin", "marketplace", "add", appDir()]);
  if (!r.ok && !/already|exists/i.test(r.out)) { console.log(`  Codex 插件市场添加失败：${r.out.slice(0, 300)}`); return 1; }
  r = codexCmd(["plugin", "add", `${NAME}@${NAME}`]);
  if (!r.ok && !/already|installed/i.test(r.out)) { console.log(`  Codex 插件安装失败：${r.out.slice(0, 300)}`); return 1; }
  console.log("  ✓ Codex 插件已安装");
  console.log(`  ！还差一步：Codex 的 hooks 要你亲自信任一次。下次打开 Codex 看到 “Hooks need review” 时选 “Trust all and continue”
...
export async function uninstall({ yes = false } = {}) {
  if (!(await confirm("卸载 agent-limit-retry，并恢复原来的状态栏？", yes))) return 1;
  const settings = readJSON(settingsPath(), {});
  const cfg = readJSON(file("config.json"), {});
  ...
  if (zcode.unregister(path.join(appDir(), "plugin"))) console.log("  ZCode 插件已移除");
  if (codexPresent()) { codexCmd(["plugin", "remove", `${NAME}@${NAME}`]); codexCmd(["plugin", "marketplace", "remove", NAME]); console.log("  Codex 插件已移除"); }
  console.log("已卸载。你的状态栏已恢复，数据留在 " + home() + "（可以直接删掉）。");
  return 0;
}
